...
CREATE TABLE foods (
    ...
    CONSTRAINT foods_fk_s_id FOREIGN KEY (s_id) REFERENCES shops (s_id) ON DELETE CASCADE
) COMMENT '菜品表';
...
CREATE TABLE orders (
    ...
    CONSTRAINT orders_fk_c_id FOREIGN KEY (c_id) REFERENCES customers (c_id) ON DELETE CASCADE,
    CONSTRAINT orders_fk_s_id FOREIGN KEY (s_id) REFERENCES shops (s_id) ON DELETE CASCADE
) COMMENT '订单表';
